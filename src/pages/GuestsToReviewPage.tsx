...
export default function GuestsToReviewPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F0E8]">
            ...
            <Card className="shadow-sm">
              <CardHeader className="bg-[#F9F8F6]">
                <CardTitle className="flex items-center gap-2 text-lg">
                  ...
                  {/* filtered.length counts individual guest rows; displayItems groups families */}
                </CardTitle>
              </CardHeader>
                      ...
                      <table className="w-full table-auto">
                        ...
                        <tbody className="divide-y divide-[#E8E3DB]">
                          {paginated.map(item => {
                            ...
                            // Build unified member list: head first, then family members (old model)
                            const drawerMembers = [
      ...
      <Dialog
        open={!!memberCorrectionDialog}
        onOpenChange={open => { if (!open) setMemberCorrectionDialog(null); }}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
      ...
      <Dialog
        open={!!memberRejectDialog}
        onOpenChange={open => { if (!open) setMemberRejectDialog(null); }}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
