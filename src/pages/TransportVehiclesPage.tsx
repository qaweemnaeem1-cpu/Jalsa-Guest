/**
 * /transport/vehicles — Fleet overview for Transport Department Head.
 * Shows all vehicles in this transport department with maintenance status.
...
export default function TransportVehiclesPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const { data: driversData } = await supabase
      .from('users')
      .select('id,name,phone,vehicle_type,vehicle_model,vehicle_registration,vehicle_capacity,is_available,transport_department_id')
      .eq('role', 'driver')
  ...
  return (
    <div className="flex min-h-screen bg-[#F5F0E8]">
      ...
      <main className="ml-64 flex-1">
        ...
        {/* Alerts */}
        {!loading && (overdue > 0 || dueSoon > 0) && (
          <div className="mb-6 space-y-2">
            {overdue > 0 && (
              <div className="flex items-center gap-3 bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-sm text-red-700">
                <AlertTriangle className="w-4 h-4 shrink-0" />
                <span><strong>{overdue} vehicle{overdue !== 1 ? 's' : ''}</strong> have overdue maintenance — action required.</span>
              </div>
            )}
            {dueSoon > 0 && (
              <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700">
                ...
                <span><strong>{dueSoon} vehicle{dueSoon !== 1 ? 's' : ''}</strong> have maintenance due soon.</span>
              </div>
            )}
          </div>
        )}
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-[#4A4A4A] mt-0.5">Fleet overview and maintenance status for your transport department</p>
        </div>
        ...
        <div className="bg-white rounded-xl border border-[#E8E3DB]">
          ...
          {loading ? (
            ...
            <div className="p-8 text-center text-[#4A4A4A] text-sm">No drivers in this transport department.</div>
          ) : (
                        ...
                        <td className="px-4 py-3 text-xs whitespace-nowrap">
                          {nextDue ? (
                            <span className={`font-medium ${
                              v.maintStatus === 'overdue'  ? 'text-red-700'   :
                              v.maintStatus === 'due_soon' ? 'text-amber-700' :
                              'text-[#4A4A4A]'
                            }`}>
                              {nextDue}
                            </span>
