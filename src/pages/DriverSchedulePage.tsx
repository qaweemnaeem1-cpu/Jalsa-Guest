/**
 * /driver/schedule — Weekly availability calendar for head drivers.
 * Shows a Mon–Sun grid with all location drivers × each day.
...
interface EditTarget {
  driverId: string;
  driverName: string;
  dates: string[]; // one date for single-cell, multiple for bulk
}
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {isBulk ? `Set Schedule — ${target?.driverName} (${target?.dates.length} days)` : `Set Schedule — ${target?.driverName}`}
          </DialogTitle>
          {!isBulk && target && (
        ...
        <div className="space-y-4 py-1">
          ...
          {/* Shift times (only for on_duty / half_day) */}
...
export default function DriverSchedulePage() {
  ...
  const [schedules, setSchedules] = useState<Record<string, ScheduleEntry>>({}); // key: `${driverId}|${date}`
  const [taskCounts, setTaskCounts] = useState<Record<string, number>>({});     // key: `${driverId}|${date}`
  const [loading, setLoading]   = useState(true);

  // Selection state for bulk-set
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [driversRes, schedulesRes, tasksRes] = await Promise.all([
        supabase
          .from('users')
          .select('id,name,vehicle_type,location')
          .eq('role', 'driver')
  ...
  }, [user?.location, weekStart]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleBulkSet = () => {
    ...
    const allDriverDates: string[] = [];
    // For bulk: we set same schedule for all selected drivers × all 7 days
    // We use a synthetic target that covers all combinations
    setEditTarget({
      driverId:   [...selectedDrivers].join(','), // comma-separated for visual only
      driverName: label,
      dates:      allDates,
    });
    setEditExisting(null);
    void allDriverDates; // suppress unused warning
  };

  // ── save handler ──────────────────────────────────────────────────────────────
  const handleSaved = useCallback((entries: ScheduleEntry[]) => {
    // If bulk (multiple selected drivers), we need to save for each selected driver
