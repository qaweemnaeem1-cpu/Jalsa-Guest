...
import {
  ...
  Loader2, ChevronDown, X, Plus, Bot,
} from 'lucide-react';
...
export default function DriverAllTasksPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      setSuggested(all.filter(t => t.is_suggestion && t.status === 'suggested'));
      setActive(all.filter(t => !t.is_suggestion && (t.status === 'pending' || t.status === 'in_progress')));
                    ...
                    <tbody className="divide-y divide-[#E8E3DB]">
                      {filteredSuggested.map(task => (
                        <tr key={task.id} className="hover:bg-[#F5F0E8]/40 transition-colors">
                          ...
                          <td className="px-4 py-3 max-w-[130px] truncate font-medium text-[#1A1A1A]">{task.guest_name ?? task.delegation_name ?? '—'}</td>
              ...
              {filteredActive.length === 0 ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#E8E3DB] bg-[#F5F0E8]/30">
                        {['Driver', 'Date', 'Time', 'Type', 'Guest', 'Route', 'Status', 'Actions'].map(h => (
                          <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-[#4A4A4A] uppercase tracking-wider">{h}</th>
                        ))}
                      </tr>
                    </thead>
